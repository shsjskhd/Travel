...
export default function DestinationDetail() {
  ...
  if (error || !d) return <div className="flex min-h-[70vh] flex-col items-center justify-center gap-3 pt-24" data-testid="dest-detail-error"><p className="text-muted-foreground">Destinasi tidak ditemukan.</p><Link to="/destinations" className="rounded-full bg-primary px-4 py-2 text-[13px] font-semibold text-primary-foreground">Kembali ke Destinasi</Link></div>;
    ...
    <div>
      ...
      <section className="relative flex min-h-[62vh] items-end overflow-hidden bg-primary">
        ...
        <div className="relative mx-auto w-full max-w-7xl px-4 pb-12 pt-32 sm:px-6 lg:px-8">
          <Link to="/destinations" className="inline-flex items-center gap-1.5 text-[13px] font-medium text-white/80 transition hover:text-white"><ArrowLeft size={15} /> Semua destinasi</Link>
          ...
          {d.best_time ? <span className="mt-4 inline-flex items-center gap-1.5 rounded-full bg-white/15 px-3 py-1.5 text-[12.5px] text-white backdrop-blur-sm"><CalendarClock size={14} /> Waktu terbaik: {d.best_time}</span> : null}
        </div>
      </section>
      ...
      <div className="mx-auto max-w-7xl space-y-16 px-4 py-14 sm:px-6 lg:px-8">
        ...
        {route.length >= 2 ? <Reveal as="section"><ScrollStory points={route} title={`Perjalanan menuju ${d.name}`} /></Reveal> : null}
        ...
        {hotels.length ? (
          <section>
            <h2 className="flex items-center gap-2 font-fraunces text-2xl text-foreground sm:text-3xl"><Hotel size={22} /> Rekomendasi Menginap</h2>
