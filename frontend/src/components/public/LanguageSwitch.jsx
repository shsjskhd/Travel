...
export default function LanguageSwitch({ solid = true, compact = false }) {
  ...
  return (
    <div className={`inline-flex items-center gap-0.5 rounded-full border p-0.5 ${shell}`}
      ...
      {langs.map((l) => {
        ...
        return (
          <button key={l.code} type="button" onClick={() => setLang(l.code)}
            ...
            className={`rounded-full px-2.5 py-1 text-[11.5px] font-bold tracking-wide transition ${
              active
