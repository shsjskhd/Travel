    ...
    <section className="relative overflow-hidden" data-testid="package-strip">
      ...
      <div className="relative mx-auto max-w-7xl px-4 py-16 sm:px-6 lg:px-8">
        <SectionHeading
          ...
          title="Rencana perjalanan yang sudah tersusun"
          subtitle="Itinerary, unit, dan driver sudah dipaketkan — Anda tinggal memilih tanggal."
        />
        ...
        {loading ? (
          ...
          <div className="mt-10 rounded-2xl border border-dashed border-border bg-card px-5 py-12 text-center" data-testid="pkg-empty">
            <p className="text-[14px] text-muted-foreground">Belum ada paket wisata yang dipublikasikan.</p>
            <Link to="/quotation" data-testid="pkg-empty-quote" className="mt-4 inline-flex items-center gap-2 rounded-full bg-primary px-4 py-2 text-[13px] font-semibold text-primary-foreground">
              Minta itinerary khusus <ArrowRight size={14} />
            </Link>
          </div>
        ) : (
              ...
              <Reveal key={p.id} delay={i * 0.07}>
                <div className="card-premium lift flex h-full flex-col overflow-hidden rounded-2xl" data-testid={`pkg-item-${p.slug}`}>
                  ...
                  <div className="flex flex-1 flex-col p-5">
                    <h3 className="font-fraunces text-xl leading-snug text-foreground">{p.name}</h3>
                    <div className="mt-2 flex flex-wrap gap-2 text-[11.5px] font-medium text-muted-foreground">
                      <span className="inline-flex items-center gap-1.5 rounded-full bg-secondary px-2.5 py-1"><CalendarDays size={12} /> {p.days} hari</span>
                    ...
                    <div className="mt-auto pt-5">
                      <p className="text-[11px] uppercase tracking-wider text-muted-foreground">Mulai dari</p>
