...
def test_i18n(api, art):
    ...
    if not ai_ready:
        check("Bantuan terjemahan AI tersedia (EMERGENT_LLM_KEY)", False,
              "ai_available=false — endpoint harus 503 berpesan")
        r = api.post("/content/articles/translate",
                     json={"target": "en", "fields": {"title": "Sewa mobil di Bali"}})
        check("Tanpa kunci LLM endpoint membalas 503 berpesan jelas", r.status_code == 503,
